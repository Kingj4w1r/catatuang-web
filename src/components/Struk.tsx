...
export default function Struk() {
  ...
  const saldoAwal = ambilSaldoAwal()
  const now       = new Date()
    ...
    <div className="p-6 sp-page max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex gap-2">
        {([['hari','HARI INI'],['minggu','MINGGU INI'],['bulan','BULAN INI']] as [Periode,string][]).map(([v,l]) => (
          <button key={v} onClick={() => { setPeriode(v); setAnimKey(k=>k+1) }}
            className={pillCls(periode===v)}>
            {l}
          </button>
        ))}
      </div>
      ...
      <div key={animKey} ref={printRef}
        ...
        <div className="space-y-2 mb-5">
          {filtered.length === 0 ? (
            ...
            filtered.map((t, i) => (
              <div key={t.id} className="flex items-center animate-sp-fade-up"
                   style={{ animationDelay:`${i*0.04}s` }}>
                <span className="text-[12px] text-sp-silver w-6 text-right mr-3 shrink-0">
                  {i+1}
                </span>
                ...
                <span className={`text-[14px] font-bold ml-3 ${
        ...
        {/* Footer */}
        <p className="text-[10px] text-sp-silver text-center mt-6">
          CatatUang v3.0 · {new Date().getFullYear()}
        </p>
      </div>
    </div>
  )
